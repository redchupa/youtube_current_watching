...
유튜브에서 내부 코드가 변경 될때마다 디버깅 하고 코드패치 해야 되서 힘드네요. 
제가 퇴근하고 육아를 해야하고 개인적인 시간이 없어서 코드 패치할 시간이 부족하네요.
향후 유튜브에서 또 코드 변경 해서 사용이 불가능 해진다면 쉽게 패치할 수 있도록 디버깅 코드를 주석처리 해두었으니 직접 수정해서 쓰실분들은 참고하세요
