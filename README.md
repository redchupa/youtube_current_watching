...
\+ \(25/10/01\) 기능 추가 개발 요청을 받아, 구독한 채널 정보를 볼 수 있게 업데이트 하였습니다.
  육아에 도움이 되길 바래요.

\+ \(25/10/16\) 유튜브가 videoRenderer 형식에서 lockupViewModel 형식으로 변경하여 코드패치 하였습니다.
한번 개발 하고나서는 잘 사용이 가능해서 앞으로 고칠일 없을줄 알았는데,
유튜브에서 내부 코드가 변경 될때마다 업데이트 해줘야 되네요.

제가 퇴근하고 아이와 시간을 보내고 싶기도 하고 개인적인 시간이 없어서 코드 패치할 시간이 부족하네요.
유튜브에서 또 코드 패치를 한다면, 디버깅 코드를 주석 처리 해두었으니, 향후 사용이 불가능하면 직접 수정해서 써주시면 감사하겠습니다.
